...
type ActivityEvent = { id: number; eventType: string; publicSummary: string | null; internalMetadata: Record<string, unknown>; createdByName: string | null; createdAt: string }
...
function eventLabel(event: ActivityEvent) {
  ...
  switch (event.eventType) {
    ...
    case 'attachment_received': return { title: 'Anexo recebido', detail: event.publicSummary || 'Um anexo sanitizado foi vinculado ao caso.' }
...
export function CaseActivityPanel({ reportId, canAddNote, canMessageReporter }: { reportId: string; canAddNote: boolean; canMessageReporter: boolean }) {
  ...
  async function load() {
    ...
    if (error || !data) { setFeedback('Não foi possível carregar a timeline e as mensagens deste caso.'); setActivity({ events: [], messages: [] }) }
    else { const parsed = data as unknown as Partial<Activity>; setActivity({ events: parsed.events ?? [], messages: parsed.messages ?? [] }) }
    setLoading(false)
  }

  useEffect(() => { void load() }, [reportId])
  useEffect(() => { if (!canMessageReporter && visibility === 'reporter_visible') setVisibility('internal_only') }, [canMessageReporter, visibility])

  async function send() {
    if (!supabase || sending || !body.trim()) return
    setSending(true); setFeedback('')
    const { error } = await supabase.rpc('operations_add_report_message', { p_report_id: reportId, p_visibility: visibility, p_body: body.trim() })
    if (error) setFeedback(visibility === 'internal_only' ? 'Não foi possível registrar a nota interna.' : 'Não foi possível enviar a mensagem ao denunciante.')
    else { setBody(''); setFeedback(visibility === 'internal_only' ? 'Nota interna registrada.' : 'Mensagem registrada no canal seguro do denunciante.'); await load() }
    setSending(false)
  }
  ...
  return <>
    <CaseAttachmentsPanel reportId={reportId} />
    <div className="case-activity-grid">
      <article className="case-section activity-timeline-card">
        ...
        {loading ? <div className="activity-loading"><LoaderCircle className="spin" size={20} /> Carregando atividade</div> : timeline.length === 0 ? <div className="activity-empty">Nenhum evento registrado.</div> : <div className="activity-timeline">{timeline.map((event) => { const label = eventLabel(event); return <div className="activity-event" key={event.id}><span className="activity-dot" /><div><strong>{label.title}</strong><p>{label.detail}</p><small>{event.createdByName || 'Sistema'} • {formatDate(event.createdAt)}</small></div></div> })}</div>}
      </article>
      ...
      <article className="case-section activity-messages-card">
        ...
        <div className="case-messages">{activity.messages.length === 0 ? <div className="activity-empty">Nenhuma mensagem registrada.</div> : activity.messages.map((message) => <div className={`case-message ${message.authorType} ${message.visibility}`} key={message.id}><div className="case-message-meta"><strong>{message.authorName}</strong><span>{message.visibility === 'internal_only' ? <><LockKeyhole size={12} /> Nota interna</> : message.authorType === 'reporter' ? 'Denunciante' : 'Visível ao denunciante'}</span></div><p>{message.body}</p><small>{formatDate(message.createdAt)}</small></div>)}</div>
        {canAddNote && <div className="message-composer"><div className="message-kind"><button className={visibility === 'internal_only' ? 'active' : ''} onClick={() => setVisibility('internal_only')} type="button"><StickyNote size={15} /> Nota interna</button>{canMessageReporter && <button className={visibility === 'reporter_visible' ? 'active' : ''} onClick={() => setVisibility('reporter_visible')} type="button"><Send size={15} /> Mensagem ao denunciante</button>}</div><textarea maxLength={8000} onChange={(event) => setBody(event.target.value)} placeholder={visibility === 'internal_only' ? 'Registre uma observação exclusiva da equipe autorizada.' : 'Escreva uma mensagem que ficará disponível no acompanhamento por protocolo.'} rows={4} value={body} /><div className="message-composer-actions"><small>{body.length}/8000</small><button className="button primary compact" disabled={sending || !body.trim()} onClick={() => void send()} type="button">{sending ? <LoaderCircle className="spin" size={15} /> : visibility === 'internal_only' ? <StickyNote size={15} /> : <Send size={15} />} {visibility === 'internal_only' ? 'Registrar nota' : 'Enviar mensagem'}</button></div></div>}
