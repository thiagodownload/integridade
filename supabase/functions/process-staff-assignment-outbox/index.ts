...
function templateFor(assignmentType: string): MailTemplate {
  if (assignmentType === 'principal') {
    return {
      subject: 'Nova responsabilidade no Canal de Integridade',
      text: 'Você foi definido como responsável principal por um caso no Canal de Integridade.',
    }
  }
  if (assignmentType === 'observer') {
    return {
      subject: 'Novo acompanhamento disponível',
      text: 'Você recebeu acesso de acompanhamento a um caso no Canal de Integridade.',
    }
  }
  return {
    subject: 'Você foi adicionado a um caso',
    text: 'Você foi incluído como colaborador em um caso no Canal de Integridade.',
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: claimed, error: claimError } = await service.rpc('claim_staff_assignment_outbox_internal', {
      p_limit: 10,
    })
    if (claimError) return reply(500, { error: 'outbox_claim_failed' })

    const jobs = (Array.isArray(claimed) ? claimed : []) as Job[]
    if (jobs.length === 0) return reply(200, { ok: true, claimed: 0, sent: 0, failed: 0 })

    let sent = 0
    let failed = 0

    async function complete(job: Job, success: boolean, errorCode?: string) {
      await service.rpc('complete_staff_assignment_outbox_internal', {
    ...
    for (const job of jobs) {
      try {
        ...
        if (
          profileError ||
          !profile ||
          String(profile.organization_id) !== job.organization_id ||
          !profile.active ||
        ...
        const template = templateFor(job.assignment_type)
        try {
          ...
          const code = error instanceof Error && (
            error.message === 'email_transport_not_configured' ||
            error.message === 'email_transport_load_failed'
          ) ? 'email_transport_unavailable' : 'smtp_delivery_failed'
          throw new Error(code)
        }

        await complete(job, true)
        await audit(job, 'notification.staff.assignment.email.sent', { attempt: job.attempts })
        sent += 1
      } catch (error) {
        const code = error instanceof Error ? error.message : 'delivery_failed'
        const safeCode = ['recipient_unavailable','email_transport_unavailable','smtp_delivery_failed'].includes(code)
          ? code
          : 'delivery_failed'
        try {
          await complete(job, false, safeCode)
          await audit(job, 'notification.staff.assignment.email.failed', {
            attempt: job.attempts,
            error_code: safeCode,
          })
        } catch {
          // O item permanece deslocado por available_at e será retomado em execução futura.
        }
        failed += 1
      }
    }
